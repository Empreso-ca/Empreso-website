...
export const FileUpload = ({
  ...
  // Update this function to only keep the latest file
  const handleFileChange = (newFiles: File[]) => {
    const latestFile = newFiles[0]; // Get only the first file
    if (latestFile) {
      setFiles([latestFile]); // Replace the state with the new file
      // eslint-disable-next-line @typescript-eslint/no-unused-expressions
      onChange && onChange([latestFile]); // Call onChange with the new file
    }
  };
